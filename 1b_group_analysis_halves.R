...
assign("erp_data", lapply(windows, 
                          function(x) lapply(1:length(parts), 
                                             function(y)
  read.table(paste0("erp/", "all_", x, "_half_", y, ".txt"), header=TRUE) %>% 
    mutate(window = x,
           part = parts[y]))))
...
erp_data <- erp_data %>%
  mutate(cond = ifelse(bini %in% c("3", "4", "5"), "SM",
         ...
         part = factor(part, levels = parts, labels = parts),
         distribution = ifelse(chlabel %in% c("Fz","Cz","Pz"), "center", 
                               ifelse(chlabel %in% c("LF","RF","LP","RP"), 
                                      "butterfly", "other")))

# write.csv(erp_data, "erp_data_half.csv")

## Within-group analysis ##

# Run ANOVAs in each time window (2) and distribution (2) with part as a factor
...
# Show significant results with part
# View(win_result %>% dplyr::filter(str_detect(iv, "part") == TRUE & str_detect(iv, "cond") == TRUE))
...
btx_table <- lapply(windows, function(x) lapply(analyses, function(y) 
  aov_ez(id="ID", dv="value", 
         ...
         within=c("chlabel", "cond", "part"), between=c("group"))))
...
# Show significant results with part
# View(btx_result %>% dplyr::filter(str_detect(iv, "part") == TRUE & str_detect(iv, "cond") == TRUE))
...
win_result <- win_result %>%
  dplyr::filter(str_detect(iv, "cond") == TRUE & str_detect(iv, "part") == TRUE) %>%
